...
static void load_layer(World *world, tmx_layer *current);
static void load_object_layer(World *world, tmx_layer *current);
static void load_image_layer(World *world, tmx_layer *current);
...
Level level_load(char *filename, AssetManager assets, size_t data_size) {
	...
	while(current_layer != NULL) {
		...
		case L_NONE:
			break;
		case L_LAYER:
			load_layer(&world, current_layer);
			break;
		case L_OBJGR:
			load_object_layer(&world, current_layer);	
			break;
		case L_IMAGE:
			load_image_layer(&world, current_layer);
			break;
		}
	}
}
